...
class Button:
    def __init__(self, parent, app, btn_type, label, color, size="large"):
        ...
        self.label = label
        self.size = size
        self.button = tk.Button(self.parent, bg=color, text=self.label, font=("Arial", 10), width=button_sizes[size], highlightthickness=0, border=0, command=self.onclick)
        self.x = 0
        self.y = 0
        self.visible = True
    ...
    def onclick(self):
        ...
        self.app.main_view.canvas.unbind("<B1-Motion>")
        self.app.main_view.canvas.unbind("<ButtonRelease-1>")

        self.handlers.get(self.btn_type, lambda: None)()

    # STATE METHODS
    def __handle_add_vertex(self):
        self.app.main_view.canvas.bind("<Button-1>", self.app.create_vertex)
    
    def __handle_add_edge(self):
        self.app.main_view.canvas.bind("<Button-1>", self.app.create_edge)

    def __handle_move_vertex(self):
        self.app.main_view.canvas.bind("<Button-1>", self.app.start_move_vertex)
        self.app.main_view.canvas.bind("<B1-Motion>", self.app.move_vertex)
        self.app.main_view.canvas.bind("<ButtonRelease-1>", self.app.stop_move_vertex)

    def __handle_dijkstra(self):
        if self.app.vertices and self.app.edges:
            self.app.main_view.infobox.clear()
            self.app.main_view.infobox.log("Vyber začiatočný a konečný vrchol kliknutím")
            self.app.main_view.canvas.bind("<Button-1>", self.app.visualize_dijkstra)
        self.app.close_dropdown(self.app.main_view.algorithm_dropdown)
        self.app.close_dropdown(self.app.main_view.algorithm_info_dropdown)

    def __handle_prim(self):
        if self.app.vertices and self.app.edges:
            self.app.main_view.infobox.clear()
            self.app.main_view.infobox.log("Vyber začiatočný vrchol kliknutím")
            self.app.main_view.canvas.bind("<Button-1>", self.app.visualize_prim)
        self.app.close_dropdown(self.app.main_view.algorithm_dropdown)
        self.app.close_dropdown(self.app.main_view.algorithm_info_dropdown)
    ...
    def __handle_dfs(self):
        if self.app.vertices and self.app.edges:
            self.app.main_view.infobox.clear()
            self.app.main_view.infobox.log("Vyber začiatočný vrchol kliknutím")
            self.app.main_view.canvas.bind("<Button-1>", self.app.visualize_dfs)
        self.app.close_dropdown(self.app.main_view.algorithm_dropdown)
        self.app.close_dropdown(self.app.main_view.algorithm_info_dropdown)
    ...
    def __handle_bfs(self):
        if self.app.vertices and self.app.edges:
            self.app.main_view.infobox.clear()
            self.app.main_view.infobox.log("Vyber začiatočný vrchol kliknutím")
            self.app.main_view.canvas.bind("<Button-1>", self.app.visualize_bfs)
        self.app.close_dropdown(self.app.main_view.algorithm_dropdown)
        self.app.close_dropdown(self.app.main_view.algorithm_info_dropdown)
    ...
    def __handle_euler_path(self):
        if self.app.vertices and self.app.edges:
            self.app.main_view.infobox.clear()
            self.app.main_view.infobox.log("Vyber začiatočný vrchol kliknutím")
            self.app.main_view.canvas.bind("<Button-1>", self.app.visualize_eulerian_path)
        self.app.close_dropdown(self.app.main_view.algorithm_dropdown)
        self.app.close_dropdown(self.app.main_view.algorithm_info_dropdown)
    ...
    def __handle_show_algorithms(self):
        self.app.main_view.algorithm_dropdown.change_dropdown_state()

    def __handle_show_algorithms_info(self):
        self.app.main_view.algorithm_info_dropdown.change_dropdown_state()
