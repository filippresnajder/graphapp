...
class EditMenu:
    ...
    def render_add_edge_menu(self, event, v1, v2):
        ...
        is_oriented_button = tk.Checkbutton(popup, variable=popup.is_oriented, text="Hrana je orientovaná", 
        ...
        is_oriented_button.pack(pady=5)
    ...
    def render_edge_edit_menu(self, event, edge):
        ...
        def save():
            if popup.edit_all_edges_var.get() == 1:
                self.app.update_all_edges(line_col_button["bg"], box_col_button["bg"], text_col_button["bg"])
                popup.destroy()
                return
            res = edge.update(entry.get(), line_col_button["bg"], box_col_button["bg"], text_col_button["bg"])
            if res == "weight-error":
                messagebox.showerror(parent=popup, title="Chyba", message="Váha hrany musí byť číselná hodnota.")
                return
            popup.destroy()
    ...
    def render_vertex_edit_menu(self, event, vertex):
        ...
        def save():
            if popup.edit_all_vertices_var.get() == 1:
                self.app.update_all_vertices(change_fill_color["bg"], change_outline_color["bg"], change_text_color["bg"])
                popup.destroy()
                return
            vertex.update(str(entry.get()), change_fill_color["bg"], change_outline_color["bg"], change_text_color["bg"])
            popup.destroy()
